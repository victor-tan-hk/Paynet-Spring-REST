...
public interface BasicRepository extends PagingAndSortingRepository<Developer, Integer> {
  ...
  @Query(value = "SELECT * FROM developers WHERE age <= :devage AND LOCATE(:devlang,languages) > 0", nativeQuery = true)
  List<Developer> whoAreTheCoolDevs(@Param("devage") Integer devage, @Param("devlang") String devlang);

  @Query(value = "SELECT COUNT(*) FROM developers WHERE  LOCATE(:devlang,languages) > 0", nativeQuery = true)
