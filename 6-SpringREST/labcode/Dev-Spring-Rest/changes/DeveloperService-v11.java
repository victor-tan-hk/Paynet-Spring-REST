...
public class DeveloperService implements DeveloperDAO {
  ...
  public List<Developer> getWithParams(Map<String, String> allParams) {
    ...
    if (allParams.size() == 0)
      ...
      for (Map.Entry<String, String> entry : allParams.entrySet()) {
        ...
        if (entry.getKey().equals(LANGUAGE_PARAM)) {
          ...
          if (!(tempValue.contains(AGE_PARAM)))
            throw new IncorrectJSONFormatException("Conditional expression only applicable for age parameter");

          foundValidKey = true;
          listToFilter = filterOnAge(entry.getKey(), listToFilter);
        }
        else if (entry.getKey().equals(LIMIT_PARAM)) {
          
          String tempString = entry.getValue();
          try {
            limitValue = Integer.parseInt(tempString);
          } catch (NumberFormatException ex) {
            throw new IncorrectJSONFormatException("Limit query parameter must have numeric value");
          }          
          if (startValue > 0) {
            foundValidKey = true;
            listToFilter = filterOnLimitAndStart(limitValue, startValue, listToFilter);
          }
        }
        else if (entry.getKey().equals(START_PARAM)) {
          
          String tempString = entry.getValue();
          try {
            startValue = Integer.parseInt(tempString);
          } catch (NumberFormatException ex) {
            throw new IncorrectJSONFormatException("Start query parameter must have numeric value");
          }          
          if (limitValue > 0) {
            foundValidKey = true;
            listToFilter = filterOnLimitAndStart(limitValue, startValue, listToFilter);
      ...
      // if only one of the start or limit query parameters are specified without the other
      // throw an Exception
      
      if ((limitValue > 0 && startValue == 0) || (startValue > 0 && limitValue == 0)) {
        throw new IncorrectJSONFormatException("Start and limit query parameters must be supplied together");
      }
  ...
  private List<Developer> filterOnLimitAndStart(int limitValue, int startValue, List<Developer> listToFilter) {
    
    logger.info("Filtering on limit : " + limitValue + " and start : " + startValue);
    
    if (startValue > listToFilter.size())
      throw new IncorrectJSONFormatException("The start value is larger than the list size");

    List<Developer> tempList = new ArrayList<Developer>();
    int numToInclude = 0;

    for (int i = startValue - 1; i < listToFilter.size(); i++) {
      numToInclude++;
      tempList.add(listToFilter.get(i));
      if (numToInclude == limitValue)
        break;
    }
